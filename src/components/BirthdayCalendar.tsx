        ...
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-4 sm:p-6 border border-blue-100">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {upcomingBirthdays.map(member => (
              <div key={member.id} className="bg-white rounded-lg p-3 sm:p-4 shadow-sm">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h4 className="font-medium text-gray-900 text-sm sm:text-base">{member.full_name}</h4>
          ...
          <div className="flex items-center justify-between flex-wrap gap-2">
            ...
            <div className="flex items-center space-x-2">
              ...
              <button
                ...
                className="px-3 py-1.5 sm:px-4 sm:py-2 text-xs sm:text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-medium"
              >
                Today
              </button>
              <button
          ...
          <div className="grid grid-cols-7 gap-1">
            {calendarDays.map(day => {
              ...
              return (
                <div
                  key={day.toISOString()}
                  className={`min-h-[80px] sm:min-h-[100px] p-1 sm:p-2 border rounded-lg transition-all hover:shadow-sm ${
                  ...
                  {dayBirthdays.length > 0 && (
                    <div className="space-y-0.5 sm:space-y-1">
                      {dayBirthdays.map(member => (
                        <div
                          key={member.id}
                          className="bg-gradient-to-r from-purple-100 to-pink-100 text-purple-800 text-xs px-1 sm:px-2 py-0.5 sm:py-1 rounded-md truncate flex items-center space-x-1 border border-purple-200 shadow-sm"
                          title={`${member.full_name}'s Birthday`}
                        >
                          <Gift size={8} className="text-purple-600 flex-shrink-0 hidden sm:block" />
                          <span className="truncate font-medium">{member.full_name}</span>
                        </div>
                      ))}
                      {dayBirthdays.length > 2 && (
                        <div className="text-xs text-purple-600 font-medium text-center hidden sm:block">
